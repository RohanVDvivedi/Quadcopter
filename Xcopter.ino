...
float KvzrealP = 300.0,KvzrealI = 270.0,KvzrealD = 20.0;
float KAltitP = 1.0;
...
void gethorizonypr(Quaternion q)
{
  ...
  if( zcos > 1 ){ zcos = 1; }else if( zcos < 0 ){ zcos = 0; }
...
void loop() {
    ...
    if( mpuInterrupt )
    {
      ...
      { 
        ...
        if( yprRate[0] <= 1 && yprRate[0] >= -1 ){yprRate[0] = 0;}
        ...
        if( turnoff == 0 )
        {
         ...
         if( Throttle > 970 )
         {
              ...
              YawRateErrorForDerivative   = ( + yprRate[0] /* - YawReq   */ );
 ...
 * which is obviously fucking never possible, for your stupid lowcost mother fucking quadcopter
 * so use it at your own risk bitches... :p
